...
| Windows runtime path | Preferred Windows runtime target plus the existing Rust-backed CLI slice | `rust-backed` | [`contract-inventory.md#launcher-path`](./contract-inventory.md#launcher-path) | The preferred Windows runtime target now exists for representative help/version/config/export/transform flows through `//packages/launcher:windows_slic3r` and `windows_launcher_smoke`; shared parity evidence and packaging-visible Windows behavior remain deferred |
