...
| --- | --- | --- | --- | --- |
| CLI behavior | `packages/legacy-slic3r/slic3r.pl` and related legacy native surfaces | `in progress` | [`contract-inventory.md#cli-behavior`](./contract-inventory.md#cli-behavior) | The `--version` and `--help` slices are verified through shared fixtures; broader CLI behavior remains legacy-only |
