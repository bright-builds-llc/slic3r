...
| Geometry, repair, merge, split, and layout operations such as `--repair`, `--cut`, `--cut-grid`, `--split`, `--merge`, `--info`, `--print-center`, and `--dont-arrange` | `packages/legacy-slic3r/slic3r.pl` | `//:legacy_oracle_smoke` only proves the retained CLI entry path exists, not these behaviors specifically | `source-plus-deferred` | Inventoried now; implementation deferred to later CLI parity phases | `packages/legacy-slic3r/src/test/GUI/test_cli.cpp` exercises layout behavior such as `--center` and `--dont-arrange` | These behaviors are visible contracts, but the trusted oracle does not yet exercise them end to end | `packages/slic3r-rust` |
...
| Repair, cut, and split output naming conventions such as `_fixed.obj`, `_upper.stl`, `_lower.stl`, and numbered split files | `packages/legacy-slic3r/slic3r.pl` | `//:legacy_oracle_smoke` does not exercise output creation | `source-only` | Inventoried now; still legacy-only | The implementation itself encodes these filename conventions in the retained CLI path | These utility outputs need dedicated fixtures before any Rust-backed status claims | `packages/slic3r-rust` |
