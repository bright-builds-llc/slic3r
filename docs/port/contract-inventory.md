...
| Geometry, repair, merge, split, and layout operations such as `--repair`, `--cut`, `--cut-grid`, `--split`, `--merge`, `--info`, `--print-center`, and `--dont-arrange` | `packages/legacy-slic3r/slic3r.pl` | `//:legacy_oracle_smoke` only proves the retained CLI entry path exists, not these behaviors specifically | `source-plus-deferred` | Phase 13 moves the scoped single-input `--info`, `--repair`, and `--split` slice into the Rust-backed path; merge, cut, cut-grid, and layout remain deferred | `packages/legacy-slic3r/src/test/GUI/test_cli.cpp` exercises layout behavior such as `--center` and `--dont-arrange` | The trusted oracle still does not exercise these behaviors end to end, so fixture-backed proof remains a later phase | `packages/slic3r-rust` |
...
| Repair, cut, and split output naming conventions such as `_fixed.obj`, `_upper.stl`, `_lower.stl`, and numbered split files | `packages/legacy-slic3r/slic3r.pl` | `//:legacy_oracle_smoke` does not exercise output creation | `source-only` | Phase 13 moves the scoped `_fixed.obj` and numbered split-file naming slice into the Rust-backed path; cut outputs remain deferred | The implementation itself encodes these filename conventions in the retained CLI path | These utility outputs still need fixture-backed proof before any verified status claims | `packages/slic3r-rust` |
| Layered SVG, SLA SVG, and exported artifact creation patterns | `packages/legacy-slic3r/src/test/GUI/test_cli.cpp` | `//:legacy_oracle_smoke` does not exercise output creation | `source-plus-deferred` | Inventoried now; still legacy-only | Legacy CLI tests assert numbered SVG output for slice export and single-SVG behavior for SLA flows | Output-content parity is later work; Phase 4 records the contract surface only | `packages/slic3r-rust` |
