...
- For `.planning/phases/*/*-SUMMARY.md`, keep the YAML frontmatter field
  `requirements-completed` in sync with the requirement IDs materially
  completed by that summary.
- Use the exact hyphenated key `requirements-completed`; the audit tooling reads
  that field name directly.
- Use `requirements-completed: []` when a plan summary closes no mapped
  milestone requirements.
- Do not run `mdformat` over phase `*-SUMMARY.md` files in this repo; verify
  those files through `summary-extract`, frontmatter integrity, and
  `git diff --check` instead so the YAML metadata stays parseable.
- When backfilling summary metadata, edit only the affected frontmatter blocks;
  do not rewrite the summary body unless the summary text itself is wrong.
