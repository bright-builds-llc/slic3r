...
This package is a metadata-only scope gate plus structural and semantic
evidence contract. It records the reviewed source identity, existing fixture
source, current expected-summary paths, semantic summary path, Rust boundary
path, evidence command, published narrow status row, broad status row, closed
structural and semantic field lists, and deferred scope.
