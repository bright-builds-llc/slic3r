...
version_expected="${3}"
help_expected="${4}"
config_save_expected="${5}"
config_load_base="${6}"
config_load_extra="${7}"
config_load_expected="${8}"
export_model="${9}"
export_gcode_expected="${10}"
transform_info_expected="${11}"
transform_repair_expected="${12}"
transform_split_stdout_expected="${13}"
transform_split_part_01_expected="${14}"
transform_split_part_02_expected="${15}"
...
rust_launcher="$(resolve_input "${rust_launcher}")"
...
temp_root="$(mktemp -d /tmp/slic3r-linux-runtime-parity.XXXXXX)"
trap 'rm -rf "${temp_root}"' EXIT

cp -f "${startup_script}" "${temp_root}/slic3r"
cp -f "${rust_launcher}" "${temp_root}/slic3r_cli"
chmod +x "${temp_root}/slic3r" "${temp_root}/slic3r_cli"

launcher_path="${temp_root}/slic3r"
...
if [[ "${help_output}" != "${expected_help_output}" ]]; then
	printf 'linux runtime help mismatch\nexpected:\n%s\nactual:\n%s\n' "${expected_help_output}" "${help_output}" >&2
	exit 1
fi
...
if [[ "${load_output}" != "${expected_load_output}" ]]; then
	printf 'linux runtime load mismatch\nexpected:\n%s\nactual:\n%s\n' "${expected_load_output}" "${load_output}" >&2
	exit 1
fi
...
if [[ "${gcode_output}" != "${expected_gcode_output}" ]]; then
	printf 'linux runtime export stdout mismatch\nexpected:\n%s\nactual:\n%s\n' "${expected_gcode_output}" "${gcode_output}" >&2
	exit 1
fi
...
if [[ "${gcode_contents}" != "${expected_gcode_contents}" ]]; then
	printf 'linux runtime export content mismatch\nexpected:\n%s\nactual:\n%s\n' "${expected_gcode_contents}" "${gcode_contents}" >&2
	exit 1
fi
...
if [[ "${info_output}" != "${expected_info_output}" ]]; then
	printf 'linux runtime info mismatch\nexpected:\n%s\nactual:\n%s\n' "${expected_info_output}" "${info_output}" >&2
	exit 1
fi
...
if [[ "${repair_output}" != "${expected_repair_output}" ]]; then
	printf 'linux runtime repair stdout mismatch\nexpected empty output\nactual:\n%s\n' "${repair_output}" >&2
	exit 1
fi
...
if [[ "${split_output}" != "${expected_split_output}" ]]; then
	printf 'linux runtime split stdout mismatch\nexpected:\n%s\nactual:\n%s\n' "${expected_split_output}" "${split_output}" >&2
	exit 1
fi
...
if [[ "${split_part_02_contents}" != "${expected_split_part_02_contents}" ]]; then
	printf 'linux runtime split part 02 mismatch\nexpected:\n%s\nactual:\n%s\n' "${expected_split_part_02_contents}" "${split_part_02_contents}" >&2
	exit 1
fi

printf 'verified linux runtime fixture\n'
